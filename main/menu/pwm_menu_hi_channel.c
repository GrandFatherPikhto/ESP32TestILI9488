...
void pwm_menu_hi_channel_init(pwm_menu_handle_t *handle) {
    lv_obj_add_event_cb(handle->menu_data->screens.hi_channel, screen_loaded_event_cb, LV_EVENT_SCREEN_LOADED, handle);
...
static void create_main_container(pwm_menu_handle_t *handle) {
    container = lv_obj_create(handle->menu_data->screens.hi_channel);
...
static void menu_update(pwm_menu_handle_t *handle) {
    ...
    if (handle->pwm_config->channel_hi.enabled) {
        lv_obj_add_state(enable_switch, LV_STATE_CHECKED);
    } else {
        lv_obj_remove_state(enable_switch, LV_STATE_CHECKED);
    }

    lv_slider_set_value(delay_slider, handle->pwm_config->channel_hi.delay, LV_ANIM_OFF);
    lv_slider_set_value(duration_slider, handle->pwm_config->channel_hi.duration, LV_ANIM_OFF);
    lv_slider_set_value(duty_slider, handle->pwm_config->channel_hi.duty, LV_ANIM_OFF);
}
...
static void delay_handler_cb(lv_event_t *event) {
    pwm_menu_handle_t *handle = lv_event_get_user_data(event);
    handle->pwm_config->channel_hi.delay = (uint16_t)lv_slider_get_value(delay_slider);
    update_delay_label(handle);
}

static void duration_handler_cb(lv_event_t *event) {
    pwm_menu_handle_t *handle = lv_event_get_user_data(event);
    handle->pwm_config->channel_hi.duration = (uint16_t)lv_slider_get_value(duration_slider);
    update_duration_label(handle);
}

static void duty_handler_cb(lv_event_t *event) {
    pwm_menu_handle_t *handle = lv_event_get_user_data(event);
    handle->pwm_config->channel_hi.duty = (uint16_t)lv_slider_get_value(duty_slider);
    if (handle->pwm_config->channel_hi.duty + handle->pwm_config->channel_lo.duty > 100) {
        handle->pwm_config->channel_lo.duty = 100 - handle->pwm_config->channel_hi.duty;
    }
    update_duty_label(handle);
}
...
static void enable_handler_cb(lv_event_t *event) {
    ...
    if (lv_obj_has_state(enable_switch, LV_STATE_CHECKED)) {
        handle->pwm_config->channel_hi.enabled = true;
    } else {
        handle->pwm_config->channel_hi.enabled = false;
    }
}
...
inline static void update_delay_label(pwm_menu_handle_t *handle) {
    lv_label_set_text_fmt(delay_label, "Delay: %u", handle->pwm_config->channel_hi.delay);
}

inline static void update_duration_label(pwm_menu_handle_t *handle) {
    lv_label_set_text_fmt(duration_label, "Duration: %u", handle->pwm_config->channel_hi.duration);
}

inline static void update_duty_label(pwm_menu_handle_t *handle) {
    lv_label_set_text_fmt(duty_label, "Duty: %u", handle->pwm_config->channel_hi.duty);
}
